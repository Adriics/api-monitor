        ...
        <div className="min-h-screen bg-zinc-950">
            ...
            <main className="max-w-4xl mx-auto px-6 py-8">
                ...
                <div className="flex items-center justify-between mb-6">
                    <h2 className="text-white font-medium text-lg">Monitores</h2>
                    <Link
                ...
                {monitors?.length === 0 && (
                    <div className="border border-zinc-800 border-dashed rounded-xl p-12 text-center">
                        <p className="text-zinc-500 text-sm">No tienes monitores todavía</p>
                        <Link
                            href="/dashboard/monitors/new"
                ...
                <div className="space-y-3">
                    {monitors?.map(monitor => (
                        <Link
                            ...
                            className="block bg-zinc-900 border border-zinc-800 rounded-xl px-5 py-4 hover:border-zinc-700 transition-colors"
                        >
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    <div className={`w-2 h-2 rounded-full ${monitor.active ? 'bg-emerald-400' : 'bg-zinc-600'}`} />
                                    <div>
                                        <p className="text-white text-sm font-medium">{monitor.name}</p>
                                        <p className="text-zinc-500 text-xs mt-0.5">{monitor.url}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="text-zinc-400 text-xs">{monitor._count.checks} checks</p>
                                    <p className="text-zinc-600 text-xs mt-0.5">cada {monitor.intervalMins} min</p>
                                </div>
                            </div>
                        </Link>
                    ))}
